<section id="sidebar">
    <a href="#" class="brand">
        <i class='bx bxs-smile'></i>
        <span class="text">Gestion universite</span>
    </a>
    <ul class="side-menu top">
        <li <?php if ($currentPage == "index.php") echo "class='active'"; ?>>
            <a href="/projet_web/index.php">
                <i class='bx bxs-dashboard'></i>
                <span class="text">Accueil</span>
            </a>
        </li>
        <li
            <?php if ($currentPage == "listes-etudiants.php" || $currentPage == "ajouter-etudiants.php") echo "class='active'"; ?>>
            <a href="/projet_web/pages/etudiants/listes-etudiants.php">
                <i class='bx bxs-shopping-bag-alt'></i>
                <span class="text">Gestion Etudiant</span>
            </a>
        </li>
        <li <?php if ($currentPage == "listes-personnels.php") echo "class='active'"; ?>>
            <a href="/projet_web/pages/personnels/listes-personnels.php">
                <i class='bx bxs-doughnut-chart'></i>
                <span class="text">Gestion Personnel</span>
            </a>
        </li>

        <li>
            <a href="#">
                <i class='bx bxs-doughnut-chart'></i>
                <span class="text">Blog</span>
            </a>
        </li>
        <li>
            <a href="#">
                <i class='bx bxs-doughnut-chart'></i>
                <span class="text">Contact</span>
            </a>
        </li>
    </ul>
    <ul class="side-menu">
        <li>
            <a href="#">
                <i class='bx bxs-cog'></i>
                <span class="text">A Propos</span>
            </a>
        </li>
        <li>
            <a href="#" class="logout">
                <i class='bx bxs-log-out-circle'></i>
                <span class="text">Déconnexion</span>
            </a>
        </li>
    </ul>
</section>